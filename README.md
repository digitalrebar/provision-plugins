...
The content layer can be provided by the `drbundler` command.  By specifying a \.go file extension as an output file, `drbundler` will generate
a go file with a global string named `contentYamlString`.

A `go generate` line can be used to assist with the construction assuming the `drbundler` is in the path.
